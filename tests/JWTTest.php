<?php

use oneid\jwt_auth\JWTAuth\Signer;
use PHPUnit\Framework\TestCase;
use oneid\jwt_auth\JWTAuth\UserInfo;

class JWTTest extends TestCase
{
    private $privateKey = <<<EOF
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
EOF;
    private $headlessPrivateKey = "";
    private $issuer = "http://www.example.com";
    private $loginURL = "https://oauth2.ci-731.account.tencentcs.com/v1/sso/jwtp/1102878596482998272/1151383032381308928/kit/{app_type}";
    private $privateKeyPath = "./private.key";

    var $signer;

    protected function prepare()
    {
        $this->signer = new Signer($this->privateKey, $this->issuer, $this->loginURL);
    }

    public function testSmallerLifetime()
    {
        $this->expectException(InvalidArgumentException::class);
        $lifetime = 0;
        $singer = new Signer($this->privateKey, $this->issuer, $this->loginURL, $lifetime);
    }

    public function testGreaterLifetime()
    {
        $this->expectException(InvalidArgumentException::class);
        $lifetime = 0;
        $singer = new Signer($this->privateKey, $this->issuer, $this->loginURL, $lifetime);
    }

    public function testInvalidKey()
    {
        $this->expectException(InvalidArgumentException::class);
        $singer = new Signer("", $this->issuer, $this->loginURL);
    }

    public function testGenerateToken()
    {
        try {
            $customAttr = array("employee_number" => "001");
            $user = new UserInfo("user_id1", "test1");
            $user->setUsername('test1')->setCustomAttributes($customAttr);
            $signer = new Signer($this->privateKey, $this->issuer, $this->loginURL);
            $token = $signer->generateToken($user);
            $this->assertNotEmpty($token);
            echo $token.PHP_EOL;
        } catch (InvalidArgumentException $e) {
            $this->fail($e->getMessage());
        }
    }

    public function testGenerateTokenWithSpecificLifetime()
    {
        try {
            $user = new UserInfo("user_id1", "test1");
            $user->setUsername('test1');
            $signer = new Signer($this->privateKey, $this->issuer, $this->loginURL, 200);
            $token = $signer->generateToken($user);
            $this->assertNotEmpty($token);
            echo $token.PHP_EOL;
        } catch (InvalidArgumentException $e) {
            $this->fail($e->getMessage());
        }
    }

    public function testGenerateTokenFromKeyFile()
    {
        try {
            $user = new UserInfo("user_id1", "test1");
            $user->setUsername('test1');
            $signer = Signer::newSignerFromKeyFile($this->privateKeyPath, $this->issuer, $this->loginURL, 200);
            $token = $signer->generateToken($user);
            $this->assertNotEmpty($token);
            echo $token.PHP_EOL;
        } catch (InvalidArgumentException $e) {
            $this->fail($e->getMessage());
        }
    }

    public function testGenerateTokenWithHeadlessKey()
    {
        try {
            $user = new UserInfo("user_id1", "test1");
            $user->setUsername('test1');
            $signer = new Signer($this->headlessPrivateKey, $this->issuer, $this->loginURL, 200);
            $token = $signer->generateToken($user);
            $this->assertNotEmpty($token);
            echo $token.PHP_EOL;
        } catch (InvalidArgumentException $e) {
            $this->fail($e->getMessage());
        }
    }

    public function testGenerateLoginURL()
    {
        try {
            $customAttr = array("employee_number" => "001");
            $user = new UserInfo("user_id1", "test1");
            $user->setUsername('test1')->setCustomAttributes($customAttr);
            $signer = new Signer($this->privateKey, $this->issuer, $this->loginURL);

            $queryParams = array("meeting_common" => "https://meeting.tencent.com/");
            $loginURL = $signer->generateLoginURL($user, App_Tencent_Meeting, $queryParams);
            $this->assertNotEmpty($loginURL);
            echo $loginURL.PHP_EOL;
        } catch (InvalidArgumentException $e) {
            $this->fail($e->getMessage());
        }
    }

    public function testGenerateLoginURLWithTokenKey()
    {
        try {
            $customAttr = array("employee_number" => "001");
            $user = new UserInfo("user_id1", "test1");
            $user->setUsername('test1')->setCustomAttributes($customAttr);
            $signer = new Signer($this->privateKey, $this->issuer, $this->loginURL,  200, "jwt");

            $queryParams = array("meeting_common" => "https://meeting.tencent.com/");
            $loginURL = $signer->generateLoginURL($user, App_Tencent_Meeting, $queryParams);
            $this->assertNotEmpty($loginURL);
            echo $loginURL.PHP_EOL;
        } catch (InvalidArgumentException $e) {
            $this->fail($e->getMessage());
        }
    }
}